#This is a script to practice using some of the pyBDP reduction methods
...
sciencePos  = np.array([[60,  10],[2081, 2055]], dtype = np.int32)
...
if not os.path.isfile(indexFile):
    ...
    fileIndex = Table([fileList, obsType, waveBand, polAng, binType, lights],
                      names = ['Filename', 'Data',  'Waveband',
...
for thisBin in uniqBins:
    ...
    if os.path.isfile(filename):
        ...
        masterBias = Bias()
        masterBias.arr = Bias.master_bias(biasImgList, clipSigma = 5.0)
    ...
    for thisAng in uniqPolAngs:
        ...
        for thisBin in uniqBins:
            ...
            if os.path.isfile(filename):
                ...
                # Select the correct polynomial order for this binning level
                polyInd            = (overscanPolyDegrees['Binning'] == thisBin)
                overscanPolyDegree = int((overscanPolyDegrees[polyInd])['Polynomial'])
                ...
                flatBins = np.ceil(0.25*(np.max(flatModeImg) -
                                         np.min(flatModeImg)))
                ...
                # Grab the data within 30 counts of the estimated mode
                flatModeData = flatModeImg[np.where(
                  np.abs(flatModeImg - flatMode) < 30)]
                xmin         = np.floor(flatMode - 30)
                xmax         = np.ceil(flatMode + 30)
                flatBins     = np.mgrid[xmin:xmax:0.5]
                
                # Use kernel density estimator to find a more accurate mode
                print('Evualuating gaussian kernel density estimator')
